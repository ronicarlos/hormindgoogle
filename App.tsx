...
const App: React.FC = () => {
  ...
  const calculateRealRisks = (metrics: Record<string, MetricPoint[]>, availableSources: Source[]): RiskFlag[] => {
      ...
      // ATENÇÃO: Removida a chave 'Total' genérica para evitar conflito com Colesterol Total/Proteína Total
      const psa = getSanitizedMetric(['PSA', 'Antígeno Prostático', 'Antigeno Prostatico', 'Prostata'], 'psa');
      const prostVol = getSanitizedMetric(['Volume Prostático', 'Volume Prostatico', 'Tamanho Próstata', 'Peso Próstata']);
      ...
      if (psa) {
          ...
          } else if (psa.value > 2.5) {
              // NOVO: Alerta de risco médio para PSA "limítrofe"
              risks.push({
                  category: 'Health',
                  level: 'MEDIUM',
                  message: `PSA em zona de atenção (${psa.value} ng/mL). Consulte um urologista preventivamente.`,
                  sourceId: psa.sourceId,
                  date: psa.date
              });
          }
  ...
  const handleBatchAddSources = async (files: File[]) => {
      ...
      // SHOW TOAST NOTIFICATION IF AT LEAST ONE FILE WAS SUCCESSFUL
      if (successCount > 0) {
          showNotification('success', `${successCount} arquivo(s) enviado(s) com sucesso! A lista foi atualizada.`);
      }
  };
  ...
  const handleSaveInputData = async (data: DailyLogData) => {
    ...
    const dbError = await dataService.addSource(project.id, newSource);
    if (dbError) {
        showNotification('error', "Erro ao salvar registro no banco de dados.");
        return;
    }
    ...
    showNotification('success', "Diário salvo com sucesso! Analisando impacto...");
  ...
  const handleAddExerciseToRoutine = async (exercise: Exercise) => {
    ...
    showNotification('success', `Exercício ${exercise.name} adicionado à rotina.`);
  };
  ...
  return (
    <div className="flex h-[100dvh] bg-white overflow-hidden dark:bg-gray-950">
      ...
      {/* TOAST NOTIFICATION */}
      {notification && (
        <div className={`fixed top-6 left-1/2 -translate-x-1/2 z-[200] px-6 py-3 rounded-xl shadow-2xl flex items-center gap-3 animate-in slide-in-from-top-4 fade-in duration-300 ${
            notification.type === 'success' 
            ? 'bg-emerald-600 text-white shadow-emerald-500/20' 
            : 'bg-red-600 text-white shadow-red-500/20'
        }`}>
            {notification.type === 'success' ? <IconCheck className="w-5 h-5" /> : <IconAlert className="w-5 h-5" />}
      ...
      <main className="flex-1 flex flex-col h-[100dvh] relative bg-white pb-20 md:pb-0 w-full min-w-0 dark:bg-gray-950">
        ...
        {/* --- DEDICATED MOBILE SOURCES VIEW HANDLER (FULL SCREEN SIDEBAR) --- */}
        ...
        {/* NEW: DESKTOP SOURCES GRID VIEW (Rendered in Main Area) */}
        {currentView === 'sources' && (
            <div className="hidden md:flex flex-1 flex-col h-full bg-gray-50 overflow-hidden dark:bg-gray-950">
                {/* Header */}
                <div className="shrink-0 bg-white border-b border-gray-200 p-6 flex justify-between items-center z-10 sticky top-0 dark:bg-gray-900 dark:border-gray-800">
                    <div>
                        ...
                        <p className="text-xs text-gray-500 font-medium mt-1 uppercase tracking-wider dark:text-gray-400">
                            Gerencie seus Exames, Fotos e Registros
                        </p>
                    </div>
                    <button 
                        onClick={() => mobileFileInputRef.current?.click()}
                        className="px-6 py-2.5 bg-black text-white rounded-xl font-bold text-xs flex items-center gap-2 hover:bg-gray-800 transition-all dark:bg-blue-600"
                    >
                ...
                <div className="flex-1 overflow-y-auto p-8">
                    {sources.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full text-gray-400">
                            ...
                            <p className="text-xs mt-2 opacity-60">Faça upload de exames ou adicione registros.</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                            {sources.map(source => (
                                <div 
                                    key={source.id}
                                    className={`group bg-white rounded-2xl border p-5 flex flex-col justify-between transition-all hover:shadow-lg hover:-translate-y-1 cursor-pointer dark:bg-gray-900 dark:border-gray-800 ${
                                        source.selected ? 'border-blue-500 ring-1 ring-blue-500 dark:border-blue-500' : 'border-gray-100 hover:border-gray-300 dark:hover:border-gray-700'
                                    }`}
                                    onClick={() => handleToggleSource(source.id)}
                                >
                                    <div className="flex items-start justify-between mb-4">
                                        <div className={`p-3 rounded-xl ${getSourceIconColor(source.type)}`}>
                                            {source.type === SourceType.USER_INPUT ? <IconDumbbell className="w-6 h-6" /> : 
                                             source.type === SourceType.PRONTUARIO ? <IconFile className="w-6 h-6" /> :
                                             <IconFile className="w-6 h-6" />}
                                        </div>
                                        {source.selected && <div className="bg-blue-500 text-white p-1 rounded-full"><IconCheck className="w-3 h-3" /></div>}
                                    </div>
                                    
                                    <div>
                                        <h3 className="font-bold text-gray-900 text-sm mb-1 line-clamp-2 leading-tight dark:text-white" title={source.title}>{source.title}</h3>
                                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-4">{source.date} • {source.type}</p>
                                    </div>

                                    <div className="flex gap-2 pt-4 border-t border-gray-50 mt-auto dark:border-gray-800">
                                        <button 
                                            onClick={(e) => { e.stopPropagation(); handleViewSummary(source); }}
                                            className="flex-1 py-2 bg-gray-50 text-gray-600 rounded-lg text-xs font-bold hover:bg-indigo-50 hover:text-indigo-600 transition-colors flex items-center justify-center gap-1 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-indigo-900/30 dark:hover:text-indigo-400"
                                        >
                                            <IconSparkles className="w-3.5 h-3.5" /> Analisar
                                        </button>
                                        <button 
                                            onClick={(e) => { e.stopPropagation(); if(source.fileUrl) window.open(source.fileUrl, '_blank'); }}
                                            className="p-2 text-gray-400 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition-colors dark:hover:bg-gray-700 dark:hover:text-gray-200"
                                        ...
                                        <button 
                                            ...
                                            className="p-2 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors dark:hover:bg-red-900/20"
                                            title="Excluir"
                                        >
