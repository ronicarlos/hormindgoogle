...
export default function App() {
  ...
  const handleUpload = async (files: File[]) => {
      ...
      // Feedback visual simples via alert ou toast seria ideal, mas aqui usamos o loading global para simplicidade
      setIsLoading(true);
      ...
      for (const file of files) {
          ...
          // Billing Log
          await dataService.logUsage(session.user.id, project.id, 'OCR', 500, 500); // Estimated tokens
          setBillingTrigger(prev => prev + 1);
      }
      
      // Reload project to refresh data and remove loading state
      loadProject(session.user.id);
  };

  const handleGenerateProntuario = async () => {
      if (!project || !session?.user) return;
      setIsLoading(true); // Reusing loading state for UI feedback
  ...
  const handleAddExercise = (exercise: Exercise) => {
      // In a real app, this would add to a workout plan. 
