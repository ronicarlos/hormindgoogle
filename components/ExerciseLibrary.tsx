...
const ExerciseLibrary: React.FC<ExerciseLibraryProps> = ({ project, onAddExercise }) => {
    ...
    return (
        ...
        <div className="absolute inset-0 flex flex-col bg-white dark:bg-gray-950 overflow-hidden">
            
            {/* HEADER RÍGIDO (flex-none) */}
            <div className="flex-none bg-white z-30 border-b border-gray-100 shadow-sm w-full dark:bg-gray-900 dark:border-gray-800">
            ...
            <div 
                className="flex-1 overflow-y-auto overflow-x-hidden min-h-0 bg-gray-50 custom-scrollbar dark:bg-gray-950 relative"
                onScroll={handleScroll}
                ref={listRef}
            >
                <div className="p-2 md:p-8 pb-32">
                    {isLoading ? (
                        <div className="flex items-center justify-center h-64">
                            <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-black dark:border-white"></div>
                        </div>
                    ) : (
                        <div className="max-w-7xl mx-auto grid grid-cols-2 xs:grid-cols-3 md:grid-cols-5 gap-2 md:gap-4 lg:gap-6 w-full">
                            {displayedExercises.map(ex => (
                                <div 
                                    key={ex.id} 
                                    onClick={() => setSelectedExercise(ex)}
                                    className="bg-white rounded-xl border border-gray-100 overflow-hidden shadow-sm active:scale-95 touch-manipulation transition-transform cursor-pointer group flex flex-col h-full w-full dark:bg-gray-900 dark:border-gray-800"
                                >
                                    {/* IMAGE AREA */}
